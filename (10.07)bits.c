...
int allOddBits(int x) {
    int temp1 = 0xAA;
    int temp2 = (temp1 << 8) + 0xAA;//注意：位移优先级不如加减乘除
    int temp3 = (temp2 << 8) + 0xAA;
    int temp4 = (temp3 << 8) + 0xAA;//get 0xAAAAAAAA;
    int clear_x = x & temp4;//clear all even places
    int flag = clear_x ^ temp4;
    return !flag;//if all zero then right ,otherwise,false
}
 ...
 */
int negate(int x) {
  /*通过探索得到这个规律：对于不是0或者Tmin:取反加一,对于0，依然得到0，对于Tmin:得不到*/
  return (~x+1);
}
//3
/* 
...
int isLessOrEqual(int x, int y) {
    /*思路：可以尝试用后者减去前者，再观察符号位,但是要考虑溢出的问题。因此还要附加符号的直接判断*/
    int o_x = !!(x >> 63);//获取x的符号位，如果是1就得到1，0就是0
    int o_y = !!(y >> 63);//同上
    //思路：1.考虑o_x为1并且o_y为0的情况，作为溢出的判断
...
int logicalNeg(int x) {
  //此题没有想出来，是参考了网上的过程，才想明白的。利用了补码的性质
  //对于一个数和他的补码，当这个数不是0或者Tmin时，其补码和他的符号位或之后为1，对于0，为0，对于Tmin为1，由此得到0的特殊性，来解体
